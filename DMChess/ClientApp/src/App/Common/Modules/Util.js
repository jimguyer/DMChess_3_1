...
var Util = /** @class */ (function () {
    ...
    Util.IsCommaSpace = function (pVar, pCL) {
        if (pCL === void 0) { pCL = false; }
        return pVar === "," || pVar >= "0" && pVar <= " ";
    };
    ;
    Util.IsNum = function (pVar, pCL) {
        if (pCL === void 0) { pCL = false; }
        return !isNaN(pVar);
    };
    Util.IsNumDot = function (pVar, pCL) {
        if (pCL === void 0) { pCL = false; }
        return pVar === "." || pVar >= "0" && pVar <= "9";
    };
    ;
    Util.IsPercent = function (pVar, pCL) {
        if (pCL === void 0) { pCL = false; }
        return this.IsNum(pVar.toString().substr(0, pVar.length - 1)) && pVar.toString().substr(pVar.length - 1, 1) === "%";
    };
    Util.IsChaAlpha = function (pVar, pCL) {
        if (pCL === void 0) { pCL = false; }
        return pVar >= "A" && pVar <= "Z" || pVar >= "a" && pVar <= "z";
    };
    ;
    Util.IsSide = function (pVar, pCL) { return (pVar === "T" || pVar === "R" || pVar === "B" || pVar === "L"); };
    Util.ChaUpper = function (pVar, pCL) {
        if (pCL === void 0) { pCL = false; }
        return pVar >= "A" && pVar <= "Z";
    };
    ;
    Util.ChaLower = function (pVar, pCL) {
        if (pCL === void 0) { pCL = false; }
        return pVar >= "a" && pVar <= "z";
    };
    ;
    Util.Bool = function (pEvent, pObj) {
    ...
    Util.Img = function (pEvent, pObj) {
        ...
        switch (pEvent) {
            ...
            case "CropStep":
                ...
                var bs = pObj.BaseSize;
                var cs = pObj.CropSize;
                var step = pObj.Step, stepX2 = pObj.Step * 2;
                //*****************************************************************************************************************************
                ...
                switch (pObj.Crop) {
                    case "CropLeft":
                        if (m.L > 0)
                            cs.X = m.L > stepX2 ? cs.X - stepX2 : bs.X;
                        else {
                            cs.Y += step;
                            cs.S -= stepX2;
                        }
                    ...
                    case "CropUp":
                        if (m.T > 0)
                            cs.Y = m.T > stepX2 ? cs.Y - stepX2 : bs.Y;
                        else {
                            cs.X += step;
                            cs.S -= stepX2;
                        }
                    ...
                    case "CropRight":
                        if (m.R > 0)
                            cs.X = m.R > stepX2 ? cs.X += stepX2 : cs.X + m.R;
                        else {
                            cs.X += stepX2;
                            cs.Y += step;
                            cs.S -= stepX2;
                    ...
                    case "CropDown":
                        if (m.B > 0)
                            cs.Y = m.B > stepX2 ? cs.Y += stepX2 : cs.Y + m.B;
                        else {
                            cs.X += step;
                            cs.Y += stepX2;
                            cs.S -= stepX2;
                    ...
                    case "CropPlus":
                        if (m.L > 0 && m.T > 0) { //console.log("Not docked to Left or Top");
                            if (m.L >= step && m.T >= step) {
                                cs.X -= step;
                                cs.Y -= step;
                                cs.S += step;
                            } //console.log("Left & Up Full step");
                            else {
                                if (m.L < m.T) {
                                    cs.X -= m.L;
                                    cs.Y -= m.L;
                            ...
                            } //console.log("Left & Up partial based on Left");
                        }
                        else if (m.L > 0) {
                            if (m.L >= step) {
                                cs.X -= step;
                        ...
                        } //console.log("Left Full/Partial Step");
                        else if (m.T > 0) {
                            if (m.T >= step) {
                                cs.Y -= step;
                                cs.S += step;
                        ...
                        } //console.log("Up Full/Partial Step");
                        //*****************************************************************************************************************************
                        m = this.Img("GetCropMargins", { BaseSize: pObj.BaseSize, CropSize: pObj.CropSize });
                        //*****************************************************************************************************************************
                        if (m.R > 0 && m.B > 0) {
                            if (m.R >= step && m.B >= step)
                                cs.S += step;
                            else
                                cs.S = m.R < m.B ? cs.S += m.R : cs.S += m.B;
                        } //console.log("Down & Right Full/Partial Step");
                        else if (m.R > 0) {
                            if (m.R >= step) {
                                cs.Y -= step;
                                cs.S += step;
                        ...
                        } //console.log("Right Full/Partial Step");
                        else if (m.B > 0) {
                            if (m.B >= step) {
                                cs.X -= step;
                                cs.S += step;
            ...
            case "GetCropSrc":
                ...
                var wMult = pObj.RawImg.width / pObj.BaseSize.W;
                var hMult = pObj.RawImg.height / pObj.BaseSize.H;
                canvas = document.createElement('canvas');
                canvas.width = pObj.CropSize.S * wMult;
                canvas.height = pObj.CropSize.S * hMult;
                var cut = { X: (pObj.CropSize.X - pObj.BaseSize.X) * wMult, Y: (pObj.CropSize.Y - pObj.BaseSize.Y) * hMult };
                context = canvas.getContext('2d');
                context.drawImage(pObj.RawImg, cut.X, cut.Y, canvas.width, canvas.height, 0, 0, canvas.width, canvas.height);
            ...
            case "GetRotateSize":
                ...
                if (maxRatio > imgRatio) {
                    size.W = pObj.Max.W;
                    size.H = pObj.Max.W * imgRatio;
                    size.X = pObj.Max.X;
                } // Proportionally too Wide
                else {
                    size.W = pObj.Max.H / imgRatio;
                    size.H = pObj.Max.H;
                    size.X = pObj.Max.X + (pObj.Max.W - size.W) / 2;
                } // Proportionally too Tall
    ...
    Util.Phone = function (pEvent, pObj) {
        ...
        switch (pEvent) {
            ...
            case "Digits":
                ...
                for (var x = 0; x < pNumber.length; x++) {
                    ...
                    if (digits == "" && char == "1")
                        continue;
                    if (48 <= pNumber.charCodeAt(x) && pNumber.charCodeAt(x) <= 57)
                        digits += char;
                }
            ...
            case "Expand":
                ...
                if (pDigits_Length != 0)
                    number = "1(";
                if (pDigits_Length > 0)
                    number += pDigits.substr(0, 3);
                if (pDigits_Length == 3)
                    number += ")";
                if (pDigits_Length > 3)
                    number += ")" + pDigits.substr(3, 3);
                if (pDigits_Length == 6)
                    number += "-";
                if (pDigits_Length > 6)
                    number += "-" + pDigits.substr(6, pDigits_Length - 6);
                //console.log("Util.Phone.Expand * number=" + number);
                return number;
            default: alert("Util.Phone * Unknown pEvent" + pEvent);
        }
    };
    Util.Audio = function (pType) {
        if (Global_1.GM.AudioOn) {
            switch (pType) {
                default:
                case "Chime":
                    Default_1.Dft.Audio.Chime.play();
                    return;
                case "Click":
                    Default_1.Dft.Audio.Click.play();
                    return;
                case "Error":
                    Default_1.Dft.Audio.Error.play();
                    return;
                case "Ping":
                    Default_1.Dft.Audio.Ping.play();
                    return;
            }
        }
    };
    ;
    Util.Time = function (pEvent, pObj) {
